...
export default function Home(){
 ...
 return <main className="home">
  <nav className="nav"><Link href="/" className="brand">ZENO<span>STORE</span></Link><div className="navLinks"><Link href="/shop">Shop</Link><Link href="/wishlist">♡ {wishCount>0&&wishCount}</Link><Link href="/cart">Cart {cartCount>0&&`(${cartCount})`}</Link>{status==="authenticated"?<Link href="/account">Account</Link>:<Link href="/login">Login</Link>}</div></nav>
